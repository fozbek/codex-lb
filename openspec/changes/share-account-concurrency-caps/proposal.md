...
- Direction-aware hysteresis: replica-count increases (shares shrink — safe toward upstream) are adopted on the next refresh; decreases (shares grow) are adopted only after the lower count has been observed continuously for `proxy_account_cap_partition_scale_down_seconds` (default 60, minimum 30), so a missed heartbeat cannot transiently double every survivor's share.
