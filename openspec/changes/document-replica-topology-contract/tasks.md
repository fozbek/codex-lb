...
- [x] 3.2 Settings optimistic locking: `DashboardSettingsConflictError`; `SettingsRepository.commit_refresh` maps `StaleDataError` → 409 `settings_conflict`; API honors optional `expectedVersion`; response exposes `version`; retry-once wrapper for dashboard-auth single-field writers.
...
- [x] 4.1 Regression tests: two-replica fingerprint stamp/mismatch/warn/concurrent first boot + lifespan startup refusal; concurrent PUT /api/settings 409 at the route; stale `expectedVersion` 409; dashboard-auth writer retry; metrics bind-conflict classification and ERROR log (helper level — `prometheus_client` is absent in the test env, so the lifespan metrics branch is unreachable); guardian warning; migration round-trip with version backfill and single Alembic head.
